<template>
  <div class="Rent">
    ...
    <Properites :Properties="rentProperties" />
  </div>
  <Footer />
</template>
...
export default {
  ...
  mounted() {
    axios
      ...
      .then(response => {
        this.rentProperties = response.data;
        const fuse = new Fuse(this.rentProperties, this.options);
        this.rentProperties = fuse.search("rent");
      })
      .catch(error => {
        throw error;
      });
